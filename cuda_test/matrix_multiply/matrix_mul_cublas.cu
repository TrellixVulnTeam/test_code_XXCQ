...
// 定义测试矩阵的维度
int const A_ROW = 6;
int const A_COL = 10000000;
int const B_ROW = 10000000;
int const B_COL = 6;
...
int main()
{
    ...
    float a = 1, b = 0;
    // NOTE that cublas is column primary! So, we are actually computing C^T = B^T * A^T here
    cublasSgemm(handle,
        CUBLAS_OP_N,  //矩阵A的属性参数，不转置，按列优先 (stay unchanged)
        ...
        &b,           // beta的值
        d_C,          //结果矩阵C (acutally C^T but it is saved by columns)
    ...
    // We actually computed C^T, but it is saved by columns in CUBLAS, which means
    // copying it back to host is C that is exactly what we want.
    cudaMemcpy(h_C, d_C, sizeof(float) * A_ROW * B_COL, cudaMemcpyDeviceToHost);

    printf("Size of result matrix C = A*B:\n");
